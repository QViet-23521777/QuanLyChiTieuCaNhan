...
export default function AddExpenseScreen() {
  ...
  useEffect(() => {
    if (params?.defaultCategory) {
      const found = defaultCategories.find(c => c.label === params.defaultCategory);
  ...
  const handleAddTransaction = async () => {
    ...
    try {
      ...
      try {
        categoryResponse = await getCategoryByName(safeString(selectedCategory));
        console.log('getCategoryByName raw response:', categoryResponse);
        
        const categoryArray = safeParseArrayResponse(categoryResponse, 'Category');
        
        if (categoryArray.length === 0) {
          console.log('No category found with name:', selectedCategory);
        ...
        console.error('Error calling getCategoryByName:', categoryError);
        Alert.alert('Lỗi', 'Không thể truy cập dữ liệu danh mục. Vui lòng thử lại.');
        return;
      }
      ...
      try {
        ...
        console.log('getAccountByUserId raw response:', accountResponse);
        
        const accountArray = safeParseArrayResponse(accountResponse, 'Account');
        
        if (accountArray.length === 0) {
          console.log('No account found for userId:', userId);
          Alert.alert('Lỗi', 'Không tìm thấy tài khoản của người dùng');
          return;
        }
        ...
        console.error('Error calling getAccountByUserId:', accountError);
      ...
      if (!transaction.categoryId || !transaction.userId || !transaction.accountId) {
    ...
    <View style={styles.container}>
      <View style={styles.contentContainer}>
        ...
        <View style={styles.typeToggle}>
          <TouchableOpacity 
            style={[styles.toggleButton, transactionType === 'expense' && styles.activeToggle]}
            onPress={() => setTransactionType('expense')}
            activeOpacity={0.7}
          >
            <Text style={[styles.toggleText, transactionType === 'expense' && styles.activeToggleText]}>
              Chi Tiêu
            </Text>
          </TouchableOpacity>
          <TouchableOpacity 
            style={[styles.toggleButton, transactionType === 'income' && styles.activeToggle]}
            onPress={() => setTransactionType('income')}
            activeOpacity={0.7}
          >
            <Text style={[styles.toggleText, transactionType === 'income' && styles.activeToggleText]}>
              Thu Nhập
            </Text>
          </TouchableOpacity>
        </View>
            ...
            <ScrollView style={styles.categoriesContainer}>
              {defaultCategories.map((category, index) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.categoryItem,
                    selectedCategory === category.label && styles.selectedCategoryItem
                  ]}
                  onPress={() => {
                ...
                >
                  ...
                  <Text style={[
                    ...
                    selectedCategory === category.label && styles.selectedCategoryLabel
                  ]}>
                    {category.label}
                  </Text>
                </TouchableOpacity>
